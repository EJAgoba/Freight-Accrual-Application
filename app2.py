...
LOGO_SRC = (
   "https://raw.githubusercontent.com/EJAgoba/Freight-Accrual-Application/main/"
   "assets/CTAS_BIG.D-709d27… (1).png"
)  # replace with final logo URL if needed
header_html = f"""
<div class="app-header">
<img class="app-logo" src="{LOGO_SRC}" alt="Cintas Logo">
<div class="app-title">Accrual Re-Coding Tool</div>
<div class="app-subtitle">
   Upload A3’s Accrual / Weekly Audit workbook. References auto-load from this folder.
</div>
</div>
"""
...
def load_reference_tables():
   """
   Loads:
     - location_codes_df  (DataFrame with 'Code' column)
     - MY LOCATION TABLE
...
# ================= Redwood Accrual =================
# NOTE: PipelineRunner().run expects:
#   (accrual_df, cintas_location_table, complete_location_table, location_codes_df)
...
if file is None:
   st.info(
       f"Upload your {'Accrual .xlsx' if file_kind=='Accrual' else 'Weekly Audit (.txt/.csv/.xlsx)'} file."
   )
   st.stop()
# -------- Read upload --------
reader = UploadReader()
...
# -------- Load references --------
try:
   # IMPORTANT: first returned value is now a DataFrame (location_codes_df)
   location_codes_df, cintas_loc_tbl, complete_loc_tbl = load_reference_tables()
except Exception as e:
...
with st.spinner("Running accrual re-coding…"):
   try:
       # NOTE: pass location_codes_df (DataFrame), not a list
...
if file_kind == "Weekly Audit":
   st.markdown(
       "### Attach edited Weekly Audit file (must contain 'USD'/'USA' and 'CAD' tabs)"
   )
   edited_file = st.file_uploader(
       "Drop your edited Weekly Audit file here",
       type=["xlsx"],
       key="edited_wa",
   )
   if edited_file is not None:
       try:
           ...
           if not (usd_key and cad_key):
               ...
               st.success(
                   f"Edited workbook loaded: USD rows = {len(usd_df):,}, "
                   f"CAD rows = {len(cad_df):,}."
               )
               ...
               st.download_button(
                   ...
                   mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                   help=(
                       "Header = negative of Paid/Paid Amount; details = "
                       "Total Paid Minus Duty and CAD Tax; Account # is text."
                   ),
               )
       except Exception as e:
