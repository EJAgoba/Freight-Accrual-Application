...
class PipelineRunner:
    ...
    def run(
        ...
        # 1. Prepare codes  (make sure we always have a DataFrame with 'Code')

        # ===============================================================

        # Normalize location_codes_df to a DataFrame with a 'Code' column

        if isinstance(location_codes_df, (list, tuple)):

            # plain list/tuple -> DataFrame
            ...
            # already a DF, we'll handle column names below
        ...
        # If it doesn't have 'Code', try to map common names like 'Codes'
        ...
        def needs_text_extract(type_col: pd.Series) -> pd.Series:
            ...
            return (type_col.isna()) | (
                ...
                type_col.astype(str).str.upper().str.contains("THIRD")
