...
class PipelineRunner:
    ...
    def run(
        ...
        # ===============================================================
        # 2. PRIORITY RULE – If Org/Dest Type Code is filled, use that FIRST
        # ===============================================================
        if "Org Type Code" in accrual_df.columns and "Carrier Name" in accrual_df.columns:
            mask = (
                accrual_df["Org Type Code"].notna()
                & (accrual_df["Org Type Code"].astype(str).str.strip() != "")
                & (accrual_df["Carrier Name"].astype(str).str.lower() != "omnitrans")
            )
            accrual_df.loc[mask, "Consignor Code"] = (
                accrual_df.loc[mask, "Org Type Code"].astype(str).str.upper()
            )

        if "Dest Type Code" in accrual_df.columns and "Carrier Name" in accrual_df.columns:
            mask = (
                accrual_df["Dest Type Code"].notna()
                & (accrual_df["Dest Type Code"].astype(str).str.strip() != "")
                & (accrual_df["Carrier Name"].astype(str).str.lower() != "omnitrans")
            )
            accrual_df.loc[mask, "Consignee Code"] = (
                accrual_df.loc[mask, "Dest Type Code"].astype(str).str.upper()
            )
        ...
        def needs_text_extract(type_col: pd.Series) -> pd.Series:
            # blank OR contains "THIRD"
            return (type_col.isna()) | (
                type_col.astype(str).str.upper().str.contains("THIRD", na=False)
            )
        ...
        # (Optional: keep for debugging/inspection; not strictly needed after targeted merge)
        ...
        comb.create_combined_address_accrual(
            ...
            "Origin Addresss",   # NOTE: keeping your original column name
            "Origin City",
            "Origin State Code",
        )
        ...
        # ===============================================================
        # 5. Address merging ONLY for rows where extraction left Code empty
        #    (type blank or THIRD-PARTY stays the same)
        # ===============================================================
        merger = Merger()
        ...
        # Run merge only on the subset that still needs a code, then write back
        ...
        # If you want extracted to ALWAYS win (even if merge touched it), you can keep these.
        # With the targeted merge above, merge shouldn't touch extracted rows anyway.
