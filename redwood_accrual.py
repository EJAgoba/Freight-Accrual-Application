...
def render_redwood_accrual_ui(
   ...
   if a3_bol is None or rw_bol is None:
       ...
       return
   # ---- Normalize BOLs for matching on BOTH sides ----
   a3_df["__BOL_NORM__"] = _normalize_bol_series(a3_df[a3_bol])
   rw_df["__BOL_NORM__"] = _normalize_bol_series(rw_df[rw_bol])
   # Optional: if Redwood has exact duplicate normalized BOLs, keep the first
   # (this prevents duplicate shipments inflating the count even before the pipeline)
   rw_before = len(rw_df)
   rw_df = rw_df.drop_duplicates(subset="__BOL_NORM__")
   if len(rw_df) != rw_before:
       st.caption(f"De-duplicated Redwood by normalized BOL: {rw_before:,} → {len(rw_df):,}")
   ...
   if filtered.empty:
       st.warning("Nothing to process — all Redwood BOLs are present in A3 (after normalization).")
       return
   # ---- Normalize headers for pipeline ----
   filtered = _normalize_headers(filtered, HEADER_MAP)
   # ---- Ensure pipeline required columns exist ----
   for c in REQUIRED_PIPELINE_COLS:
       if c not in filtered.columns:
           filtered[c] = ""
   # ---- Stabilize row identity to prevent fan-out ----
   filtered = filtered.reset_index(drop=True).copy()
   filtered["__ROW_ID__"] = filtered.index.astype(int)
   # ---- Load references and de-dupe them on Loc Code (prevents many-to-many) ----
   try:
       location_codes, my_location_table, complete_loc_tbl = load_reference_tables()
   except Exception as e:
       st.error(f"Reference load error: {e}")
       return
   my_location_table  = _dedupe_by_loc_code(my_location_table)
   complete_loc_tbl   = _dedupe_by_loc_code(complete_loc_tbl)
   # ---- Run pipeline ----
   with st.spinner("Running Redwood accrual logic…"):
       try:
           ...
           return
   # ---- Collapse any fan-out back to one row per original shipment ----
   if "__ROW_ID__" in result.columns:
       before = len(result)
       result = (
           result.sort_values(["__ROW_ID__"])
                 .drop_duplicates(subset=["__ROW_ID__"], keep="first")
                 .drop(columns=["__ROW_ID__"], errors="ignore")
                 .reset_index(drop=True)
       )
       if before != len(result):
           st.caption(f"Collapsed merge fan-out: {before:,} → {len(result):,} rows")
   ...
   # ---- Revert headers back to Redwood names for export ----
   export_df = _revert_headers(result, REVERT_MAP)
   st.success(f"Done! Redwood Accrual processed **{len(export_df):,}** rows (one per original Redwood row).")
...
def _normalize_bol_series(s: pd.Series) -> pd.Series:
   """
   Normalize BOLs to maximize true matches:
     - str, upper, strip
     - remove all non-alphanumeric
     - strip leading zeros (keep one '0' if empty)
   """
   ...
   def _strip_zeros(x: str) -> str:
       if not x: return ""
       y = x.lstrip("0")
       return y if y != "" else "0"
   return t.map(_strip_zeros)
...
def _dedupe_by_loc_code(table: pd.DataFrame) -> pd.DataFrame:
   """Drop duplicate rows by Loc Code-like column to enforce 1:1 joins."""
   for cand in ["Loc Code", "Location Code", "Loc_Code"]:
       if cand in table.columns:
           return table.drop_duplicates(subset=[cand]).copy()
   return table
